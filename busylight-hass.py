...
def get_light(path: str) -> busylight_core.Light:
    ...
    logging.fatal("path %s not found in list of %d enumerated HID devices", as_bytes, len(devices))
    sys.exit(1)
    return

    # ~ # https://trezor.github.io/cython-hidapi/api.htm
    # ~ device = hid.device()
    # ~ device.open_path(str.encode(path))
    # ~ logging.fatal("device %s is a %s by %s (%s)", path, device.get_product_string(), device.get_manufacturer_string(), device.get_serial_number_string())

    # ~ hw = busylight_core.Hardware().from_hid(device)
    logging.debug("busylift_core %s", dir(busylight_core))
    logging.debug("busylight_core.hid %s", dir(busylight_core.hid))
    logging.debug("busylight_core.hardware %s", dir(busylight_core.hardware))

    # ~ dev = busylight_core.hid.Device()
    # ~ logging.info("trying open_path %s", path)
    # ~ dev.open_path(path)
    # ~ logging.info("after _open_path %s %s %s", dev, dev.is_open, dir(dev))
    # ~ handle = dev.handle
    # ~ logging.info("handle fter _open_path %s %s %s %s", handle.get_feature_report(),  dev.handle.get_serial_number_string(), dev.handle.get_manufacturer_string(), dir(dev.handle))


    # ~ light = busylight_core.Hardware.from_hid(device=dev)
    # ~ logging.info("light %s %s", light, dir(light))
    # ~ light.on((0, 0, 255))
    
    # ~ return
        
    l = busylight_core.Hardware.enumerate(by_type=busylight_core.hardware.ConnectionType.HID)
    logging.info("Hardware.enumerated %s", l)
    
    
    dev = busylight_core.hid.Device()
    from_open_path = dev.open_path(path)
    logging.info("from_open_path %s", from_open_path)
    
    
    for y in l:
        if y.path == b'1-2:1.0':
            logging.info("from_hid %s %s %s", y, y.as_dict(), dir(y))
            v  = busylight_core.Hardware.from_hid(device=y.as_dict())
            logging.info("v %s", v)
    
            return

  
    logging.info("device %s open: %s %s %s", path, d, dir(d), d._handle)
    from_handle = d._handle.open_path(str.encode(path))
    logging.info("from_handle %s", path, from_handle)
    try:
        device = d.open_path(path)
    except OSError as ex:
        logging.fatal("OSError opening %s: %s %s %s", path, ex, ex.strerror, ex.filename)
        sys.exit(3)
    logging.info("device %s open: %s %s", path, device, dir(device))


    
    lights = busylight_core.Light.all_lights()

    logging.info(f"Found {len(lights)} light(s)")

    for light in lights:
        light.on((255, 0, 0))  # Turn on red
 
    for light in lights:
        light.on((255, 255, 0))  # Turn on red
        light.off()
   
    d = busylight_core.hid.Device()
    logging.info("device %s open: %s %s %s", path, d, dir(d), d._handle)
    from_handle = d._handle.open_path(str.encode(path))
    logging.info("from_handle %s", path, from_handle)
    try:
